...
| ----------------- | ------- | ------------------------------ | ------- |
| `render_type`     | String  | When to consider page "loaded" | `load`  |
...
JavaScript rendering works with:
...
- ❌ Social endpoints (use Extract Templates)
- ❌ SERP endpoints (use Extract Templates)
- ❌ Maps endpoints (use Extract Templates)
- ❌ E-commerce endpoints (use Extract Templates)

### Driver requirements

| Driver | Rendering Support          |
| ------ | -------------------------- |
| vx6    | ❌ No rendering (HTTP only) |
| vx8    | ✅ Full rendering support   |
| vx10   | ✅ Full rendering support   |
| vx14   | ✅ Full rendering support   |

<Note>
  Enterprise plans with automatic driver selection: vx6 disables rendering automatically; vx8/vx10/vx14 enable it by default.
</Note>
...
| Essential    | 180 seconds |
| Advanced     | 240 seconds |
| Professional | 300 seconds |
| Enterprise   | Custom      |

### Resource limits

- **Memory**: 256 MB per page
- **JavaScript execution**: 30 seconds CPU time
- **Maximum redirects**: 10 per request
- **WebSocket connections**: 10 concurrent
- **Local storage**: 10 MB per domain
