...
Deterministic mode gives you full control over the sequence of actions performed in the browser, ensuring predictable and repeatable results. You manually specify each step like click, scroll, wait, or type.
...
<Note>
  Deterministic flows may break when page structure or selectors change. Monitor and update your flows as needed.
</Note>

## Available actions

Browser Actions are passed in the `browser_actions` array. Each action executes in sequence.
...
- **html**: Final DOM state after all actions
- **screenshot**: Page screenshot if requested
- **cookies**: Collected cookies if `get_cookies` was used
- **network_capture**: Captured network requests if configured
- **metadata**: Execution details including driver used and timing
