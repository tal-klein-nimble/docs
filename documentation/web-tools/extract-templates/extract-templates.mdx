...
```bash
POST https://gateway.nimbleway.com/api/v1/extract-template
```
...
| ---------- | ------ | ----------------------------------------------- | -------- |
| `template` | String | Template name (e.g., "amazon_pdp")              | Yes      |
| `params`   | Object | Template-specific params (ASINs, queries, etc.) | Yes      |
| `country`  | String | Target country code (template-dependent)        | No       |

<Note>
  Each template has unique parameter requirements. Use the [Get Template Details](/documentation/web-tools/extract-templates/details) API to review parameter schemas.
</Note>
